// Serial control for D8, D9 (Arduino Mega) @115200
// If your loads are normal LEDs/inputs (active-HIGH), set ACTIVE_LOW = false.
// For relay boards that are active-LOW, leave as true.

bool ACTIVE_LOW = false;   // <- likely false for LEDs, true for active-LOW relays

const uint8_t PINS[] = {8, 9};
...
void handle(const String& cmd){
  ...
  // parse: on/off <pin>  OR pulse <pin> <ms>
  int sp1 = cmd.indexOf(' ');
  ...
  if (op=="on" || op=="off"){
    int pin = rest.toInt();
    if (!validPin(pin)){ Serial.println(F("Bad pin. Use 8 or 9.")); return; }
